 ...
 *
 */
public class DimmerLevelChangeAction implements ImonitorControllerAction {
	...
	public String executeCommand(ActionModel actionModel) {
		...
		String commandParam = device.getCommandParam().trim();
		try{
			int commandIntValue = Integer.parseInt(commandParam);
			if(commandIntValue < 0){
				commandParam = "0";
			}
			if(commandIntValue > 100){
				commandParam = "100";
			}
		}catch (NumberFormatException e) {
			commandParam = "0";
		}
		queue.add(new KeyValuePair(Constants.ZW_DEVICE_CONTROL,commandParam));
...
//		}
	...
	public List<KeyValuePair> createImvgConfigParams(ActionDataHolder actionDataHolder) {
		...
		keyValuePairs.add(new KeyValuePair(Constants.ZW_DIMMER_LEVEL, actionDataHolder.getLevel()));
