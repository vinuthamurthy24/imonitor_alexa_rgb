...
public class PowerLimitReachedUpdateAlert extends AlertUpdateHelper 
{
	//bhavya start
	
	
	public Queue<KeyValuePair> UpdatePowerLimitReachedSuccess(Queue<KeyValuePair> alerts)
	throws Exception {
		String xml = IMonitorUtil.convertQueueIntoXml(alerts);
		IMonitorUtil.sendToController("alertsFromImvgService", "PowerLimitReachedsuccess", xml);
		Queue<KeyValuePair> queue = createResultQueue(alerts);
		return queue;
	}

	public Queue<KeyValuePair> UpdatePowerLimitReachedFailed(Queue<KeyValuePair> alerts)
	throws Exception {
		String xml = IMonitorUtil.convertQueueIntoXml(alerts);
		IMonitorUtil.sendToController("alertsFromImvgService", "PowerLimitReachedfailure", xml);
		Queue<KeyValuePair> queue = createResultQueue(alerts);
		return queue;
	}
