...
var handleGatewaySelectChange = function(ev){
	...
	initialActionRows += "<td><select class='selectActionDevice'></select></td>"
						...
						+"<td><input type='text' maxlength='3' size='3'style='display:none' class='DelayMinutes' name='dminutes' placeholder='Min'/></td>"
						+"<td><input type='text'  maxlength='3' size='3'style='display:none' class='DelaySeconds' name='dseconds'placeholder='Sec'/></td>"
	...
	}else if((rss == "scheduleadd" || rss == "scheduleedit")||(rss == "scenarioadd" || rss == "scenarioedit")){
		initialActionRows += "<td><select class='selectActionDevice'></select></td>"
			+"<td><select class='selectActionName'></select></td>"
			+"<td><select style='display:none' class='FanModevalue'/></td>"
			+"<td><input type='hidden' class='expressionValue' name='actionExpressions' />"
	...
	for(var deviceID in devices){
		...
		if(rss == "ruleadd" || rss == "ruleedit")
		{
			...
			if(	   device["deviceType"] == "IP_CAMERA" 
				...
				|| (device["deviceType"] == "Z_WAVE_AC_EXTENDER" && device["modelNumber"] != "ZXT120")
			) continue;
	...
	for(var deviceID in devices){
		...
		if(		((deviceName!="Home") && (deviceName!="Stay") && (deviceName!="Away")) 
			...
			if(device["actions"] != undefined){
				...
				if(device["deviceType"] == "DEVICE_DELAY" && (rss == "scenarioadd" || rss == "scenarioedit" || rss == "scheduleadd" || rss == "scheduleedit")) continue;
				if(deviceName == "Home")deviceName = formatMessage("general.home");
				else if(deviceName == "Stay")deviceName = formatMessage("general.stay");
				else if(deviceName == "Away")deviceName = formatMessage("general.away");
...
var handleActionNameSelectChange = function(ev){
	...
	var Delayminutes=cRow.find('.DelayMinutes');
	var Delayseconds=cRow.find('.DelaySeconds');
...
// 				CuratinValue.hide();				
	...
	Delayminutes.addClass('required');
	Delayminutes.addClass('editdisplayStar');
	Delayminutes.addClass('number');
	Delayminutes.attr('minvalue',0);
	Delayminutes.attr('maxvalue',299);
	Delayminutes.closest("tr").find("td:first-child").find("span").show();
	Delayseconds.addClass('required');
	Delayseconds.addClass('editdisplayStar');
	Delayseconds.addClass('number');
	Delayseconds.attr('minvalue',0);
	Delayseconds.attr('maxvalue',59);
	Delayseconds.closest("tr").find("td:first-child").find("span").show();
	Delayminutes.show();
	Delayseconds.show();
...
var handleActionValueChange = function(){
	...
	var Delayminutes=cRow.find('.DelayMinutes').val();
	var DelaySeconds=cRow.find('.DelaySeconds').val();
	if((Delayminutes!="" && DelaySeconds!="")&&(Delayminutes!=null && DelaySeconds!=null)){
	cValue=Delayminutes+"="+DelaySeconds;
	}
...
var handledelayminutesChange = function(){
	var cRow = $(this).parent().parent();
	var cDeviceId = cRow.find('.selectActionDevice').val();
	var cActionId = cRow.find('.selectActionName').val();
	var experssion = "";
	...
	if($.trim(cDeviceId + cActionId + cValue+DelaySeconds) != ""){
		experssion += cDeviceId + "=" + cActionId + "=" + cValue+"="+DelaySeconds;
	}
	cRow.find('input[type="hidden"]').val(experssion); 
	return true;
};
var handledelaysecondsChange = function(){
	var cRow = $(this).parent().parent();
	var cDeviceId = cRow.find('.selectActionDevice').val();
	var cActionId = cRow.find('.selectActionName').val();
	var experssion = "";
...
var handleFANModeChange = function(){
	...
	else if($(this).val()!= null)
	{
		...
		actionValueText.change();
	cValue = actionValueText.val()+":"+$(this).val();
	}
	else
	{
	...
	if(rss == "ruleadd")
	{
		...
		if(input.hasClass('required'))
		{
			...
			$('#defineActionTable tr').each(function() {
			    var delayId = $(this).find(".selectActionName").val();   
			    if(delayId==31){
			    	i++;
			    }
			    if(i>5){
			    	alert("Please select the Delay Less then or Equal to 5");
			    	$(".previous").click(toggleSteps('3'));
			    }
			 });
		}
	}
