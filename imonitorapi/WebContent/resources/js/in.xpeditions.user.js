...
this.generateGateWayHtml = function(){
	...
	if(this.gateWayCount < 2){
		for(var gateWayId in gateWays){
			...
			var backup = gateWays[gateWayId]['backup'];
			var Userinitiatedbackup = "";
		...
		var Userinitiatedfirmaware = "<a href='updateLatestFirmWare.action' class='FirmwareUpgrade' title='Click to Update The gateway With Latest Version' style='text-decoration: blink' targetid='reboot' firmwareversion='"+firmwareversion+"' FirmwareId='"+FirmwareId+"' gateWayVersion='"+gateWayVersion+"' gatewayid="
			...
			//naveen added to initiate gateway backup in home page
			
			if(backup == 1){
				
				Userinitiatedbackup = "<a href='toTakeBackUp.action' class='GatewayBackup' title='Click to take backup from gateway' style='text-decoration: blink;' targetid='backup' gatewayid="
				+gateWayId+">"
				+formatMessage("msg.ui.Userinitiatedgatewaybackup")
				+"</a>";
			}
			...
			$("#backup").html(Userinitiatedbackup);
		...
		for(var gateWayId in gateWays){
			...
			selectGateWay.append("<option>" + gateWayId + " </option>");
		...
		var reboot = "<a href='toReboot.action' class='remoteReboot' style='text-decoration: none' targetid='reboot' gatewayid="
			+gatewayidReboot+" ><button type='button' class='bt bbtn'>Reboot</button></a>";
...
this.setGateWayStatus = function(gateWayId, status, featuresEnabled,Latestversion,FirmwareId,gateWayVersion){
...
// sumit start 
this.setGateWayStatusAndMode = function(gateWayId, status,curMode,Latestversion,FirmwareId,gateWayVersion,AllOnOff,backup){
	gateWays[gateWayId]['status'] = status;
	...
	gateWays[gateWayId]['backup'] = backup;
};
...
this.generateTotalTab = function(storage,SlectedLanguage)
		{
			...
			var n=monthNames[d.getMonth()];
				...
				var TargetCast="<tr><td id='lable'>Target cost for "+ monthNames[d.getMonth()]+"</td><td id='lable'> &#8377;"+targetcost+"</td></tr>"+
				               "<tr><td id='lable'>Upto date cost</td><td id='lable'>&#8377;"+uptoTimecost+"</td></tr>";
...
this.generateHtmlForLocationInENMGMT = function(){
		...
		var TotalTab="<li  class='editlink totalrow' label='Total' labelroom='"+formatMessage("Energy.Totaltab")+"'>"+formatMessage("Energy.Total")+"</li>";
		...
		for(var i in locationTypes){		
			...
			var contentDiv = $("(<li id=accordion href='GetRoomWiseDataFromDB.action' LocationId="+LocationId+" class="+LocationId+" labelroom="+ locationType +" ></li>)").attr("labelroom",locationType);
		...
		$('#accordionList').append(tablerows);
		...
		for(var key in deviceDetails){
			if(wiseType == "" || wiseType == key){
				...
				for(var dKey in devices){
					...
					gateWays[device["gatewayid"]]['status'] = gatewayStatus[0];
					if(deviceLi !=false){
...
this.generateDeviceLiForEGM = function(device){
	...
	if((deviceType != "MODE_HOME") && (deviceType != "MODE_STAY") && (deviceType != "MODE_AWAY") && (enableDeviceListing != "0"))
	{
		...
		var powerinfo=device["dashboardpowerinfo"];
		var DeviceLable=device["deviceName"]+" "+formatMessage("Energy.InstantPowerconsumption")+":"+powerinfo;
		deviceLi = $("<li href='barchartweek.action' class='editlink "+device["deviceId"]+"'></li>").attr("tabindex", device["index"]).attr("labelroom",DeviceLable).attr("label",device["deviceName"]).attr("deviceId",device["deviceId"]);
...
this.generateDevicesHtml = function(device,isForChangeDevice){
	...
	var alertcommand = $.trim(device["alertparam"]);
	var enableDeviceListing = $.trim(device["enableList"]);
	if((deviceType != "MODE_HOME") && (deviceType != "MODE_STAY") && (deviceType != "MODE_AWAY") && (enableDeviceListing != "0") &&(deviceType !="DEVICE_DELAY"))
	{
...
this.generateDevicesHtmlForDeviceControlInMap = function(device,isForChangeDevice){
	...
	if(deviceType=="Z_WAVE_LCD_REMOTE"){
		contentDiv.append($("<a><b>This device has no Operation</b></a>"));
...
this.generateDevicesHtmlForEdit = function(device){
	//sumit start
	var modelnumber = $.trim(device["modelnumber"]);
	if((device['deviceType'] != "MODE_HOME") && (device['deviceType'] != "MODE_STAY") && (device['deviceType'] != "MODE_AWAY") && (device['deviceType'] != "Z_WAVE_ENERGY_MONITOR") && (device['deviceType'] != "DEVICE_DELAY")){
	...
	//vibhu start
	var nupdateSection = "<a title='Device configure' popuptitle='Update Configuration' href='ToNupdateDevice.action?"+params+"' class='Tonupdatedevicelink' > <img class='actionimage' src='../resources/images/nupdate.png'/> </a>";
	//vibhu end
...
//vibhu start: hiding AlertSection temporarily, until it is iMVG is ready
					.append("<li class='deviceeditli'><span>" + editSection + removeSection + AlertSection + nupdateSection + modelEditSection  + noMotionConfiguration  + cameraOrientationSection+ LCDRemoteConfiguration + switchTypeSection + configureRGB +configuresiren+configureusermode+minimoteconfig+"</span></li>")
//vibhu end
			);
	}
	...
	contentDiv.append($("<ul class='deviceeditul'><ul>")
			...
			.append("<li class='deviceeditli'><span>" + device['deviceLocation'] + "</span></li>")
//vibhu start: hiding AlertSection temporarily, until it is iMVG is ready
			.append("<li class='deviceeditli'><span>" + editSection + removeSection + AlertSection + nupdateSection + modelEditSection  + noMotionConfiguration  + cameraOrientationSection + LCDRemoteConfiguration + switchTypeSection + configureRGB +"</span></li>")
//vibhu end
			);
	}
