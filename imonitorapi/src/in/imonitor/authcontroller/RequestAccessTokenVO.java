...
public class RequestAccessTokenVO extends RequestBaseVO {
	...
	public String toString(String response_type, boolean isUseAuthorizationHeader) {
		String val = "grant_type=" + Oauth2Response.encodeURIComponent(grant_type) + 
				"&code="+ Oauth2Response.encodeURIComponent(code) ;
		if (!isUseAuthorizationHeader) {
			val += "&client_id=" + Oauth2Response.encodeURIComponent(this.getClient_id());
			if (response_type.equals(OAuth2Constant.AUTHORIZATION_CODE)) {
				val += "&client_secret=" + Oauth2Response.encodeURIComponent(this.getClient_secret());
			}
		}
