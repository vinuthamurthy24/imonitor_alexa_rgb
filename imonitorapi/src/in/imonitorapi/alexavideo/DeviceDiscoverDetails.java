...
public class DeviceDiscoverDetails {
	...
	public  Response discoverdevice(@HeaderParam("type") String type,@HeaderParam("accesstoken") String accessToken) throws Exception{
	   ...
	   LogUtil.info("access token: "+accessToken );
		...
		LogUtil.info("devicedetails for alexavideo discover"+result);
		String scenarioList = ClientController.sendToController("alexaService","discoverScenarios",customerId,tokenXml);
		...
		for (Object[] obj:objects){
			...
			HashMap<String,String> cookie;
			 if(zwtype.equalsIgnoreCase("Z_WAVE_MULTI_SENSOR")){
				// LogUtil.info("Z_WAVE_MULTI_SENSOR");
				 
				  desc="MOTION SENSOR";
				  displaycateg = new String[] {"MOTION_SENSOR"};
			  ...
			  if(zwtype.equalsIgnoreCase("Z_WAVE_AV_BLASTER")){
					LogUtil.info("Z_WAVE_AV_BLASTER");
					 
					  desc="TV";
					  displaycateg = new String[] {"TV"};
					  cookie = new HashMap<String, String>();
			          cookie.put("customerId",customerId );
			          cookie.put("gateway", obj[1].toString());
			          cookie.put("device", zwtype);
					 ...
					 LogUtil.info("Z_WAVE_DOOR_SENSOR");
					  ...
					  desc="DOOR SENSOR";
					  displaycateg = new String[] {"CONTACT_SENSOR"};
					  cookie = new HashMap<String, String>();
			          cookie.put("customerId",customerId );
			          cookie.put("gateway", obj[1].toString());
			          cookie.put("device", zwtype);
			 ...
			 else if(zwtype.equalsIgnoreCase("Z_WAVE_SWITCH")){
			 ...
			 else if (zwtype.equalsIgnoreCase("VIA_UNIT"))
	          {
	            // LogUtil.info("VIA unit block");
				 ...
				 /*else if(zwtype.equalsIgnoreCase("Z_WAVE_AV_BLASTER")){
					
					String modelNumber = (String) obj[6];
					if(modelNumber != null ){
					
					String[] models=modelNumber.split("-");
					String model=models[0];
					
					if(model.equalsIgnoreCase("8")){
						
					  desc="Video";
					  displaycateg = new String[] {"Z_WAVE_AV_BLASTER"};
					  cookie = new HashMap<String, String>();
			          cookie.put("customerId",customerId );
			          cookie.put("gateway", obj[1].toString());
			          cookie.put("device", zwtype);
