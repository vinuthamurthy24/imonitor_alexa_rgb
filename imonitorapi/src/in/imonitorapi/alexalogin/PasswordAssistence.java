...
@Path("/password")
public class PasswordAssistence {
	...
	public Response SmaplePage(@QueryParam("customer")String customer,@QueryParam("user")String user) throws  Exception 
	{
		...
		String resultXml = ClientController.sendToController("userService", "toValidateAlexaUser", userxml,customerxml);
		LogUtil.info("Method step1");
		LogUtil.info(resultXml);
		if(resultXml.equals("yes")){
			LogUtil.info("if block executed");
			String resultXml1 = ClientController.sendToController("userService", "generatePasswordAndNotifyUser",userxml,customerxml);
