	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnAjaxUpdate( oSettings )
		{
			if ( oSettings.bAjaxDataGet )
			{
				...
				/* Paging and general */
				oSettings.iDraw++;
				aoData.push( { "name": "dataTable.sEcho",          "value": oSettings.iDraw } );
				aoData.push( { "name": "dataTable.iColumns",       "value": iColumns } );
				aoData.push( { "name": "dataTable.sColumns",       "value": _fnColumnOrdering(oSettings) } );
				aoData.push( { "name": "dataTable.iDisplayStart",  "value": oSettings._iDisplayStart } );
				aoData.push( { "name": "dataTable.iDisplayLength", "value": oSettings.oFeatures.bPaginate !== false ?
					oSettings._iDisplayLength : -1 } );
				
				/* Filtering */
				if ( oSettings.oFeatures.bFilter !== false )
				{
					aoData.push( { "name": "dataTable.sSearch", "value": oSettings.oPreviousSearch.sSearch } );
					aoData.push( { "name": "dataTable.bRegex",  "value": oSettings.oPreviousSearch.bRegex } );
					for ( i=0 ; i<iColumns ; i++ )
					{
						aoData.push( { "name": "dataTable.sSearch_"+i,     "value": oSettings.aoPreSearchCols[i].sSearch } );
						aoData.push( { "name": "dataTable.bRegex_"+i,      "value": oSettings.aoPreSearchCols[i].bRegex } );
						aoData.push( { "name": "dataTable.bSearchable_"+i, "value": oSettings.aoColumns[i].bSearchable } );
					}
				}
				...
				if ( oSettings.oFeatures.bSort !== false )
				{
					...
					aoData.push( { "name": "dataTable.iSortingCols",   "value": iFixed+iUser } );
					for ( i=0 ; i<iFixed ; i++ )
					{
						aoData.push( { "name": "dataTable.iSortCol_"+i,  "value": oSettings.aaSortingFixed[i][0] } );
						aoData.push( { "name": "dataTable.sSortDir_"+i,  "value": oSettings.aaSortingFixed[i][1] } );
					}
					
					for ( i=0 ; i<iUser ; i++ )
					{
						aoData.push( { "name": "dataTable.iSortCol_"+(i+iFixed),  "value": oSettings.aaSorting[i][0] } );
						aoData.push( { "name": "dataTable.sSortDir_"+(i+iFixed),  "value": oSettings.aaSorting[i][1] } );
					}
					
					for ( i=0 ; i<iColumns ; i++ )
					{
						aoData.push( { "name": "dataTable.bSortable_"+i,  "value": oSettings.aoColumns[i].bSortable } );
					}
				}
